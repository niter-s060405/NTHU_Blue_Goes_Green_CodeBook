...
\newboolean{BlackMode}
\setboolean{BlackMode}{false} % 是否使用黑白模式（僅使用黑白之間的顏色）
...
\newcommand{\includesh}[2]{ % 支援 Shell (.sh)
  \subsection{#1}
    \lstinputlisting[language=bash]{#2}
}

\newcommand{\includebat}[2]{ % 支援 Batch (.bat)
  \subsection{#1}
    \lstinputlisting[language=bash]{#2}
}
...
% **************************************************
\newcommand{\NormalColor}{black} % 一般文字與符號顏色 (Color for normal words & symbols like '(', '<')

\lstset{
basicstyle=\scriptsize\ttfamily\color{\NormalColor},        % 基礎字型、大小與符號顏色 (Symbols like '(', '<' inherit this color)
numbers=left,                                               % where to put the line-numbers
numberstyle=\tiny\color{black},                             % 行號顏色 (Line numbers color)
...
keywordstyle=\bfseries\color{Blue1},                        % 關鍵字顏色 (Keywords)
commentstyle=\itshape\color{Red4},                          % 註解顏色 (Comments)
stringstyle=\itshape\color{Green4},                         % 字串顏色 (Strings)
identifierstyle=\color{\NormalColor},                       % 一般文字/變數/識別碼顏色 (Normal words / identifiers)
}

\ifthenelse{\boolean{BlackMode}}{
  \selectcolormodel{gray}
  \lstset{
    basicstyle=\scriptsize\ttfamily\color{\NormalColor},
    numberstyle=\tiny\color{\NormalColor},
    keywordstyle=\bfseries\color{black},
    commentstyle=\itshape\color{black!80},
    stringstyle=\itshape\color{black!80},
    identifierstyle=\color{\NormalColor},                          % 黑白模式下一般文字與符號顏色
  }
}{}
